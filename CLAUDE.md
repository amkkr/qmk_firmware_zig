...
- 対象キーボード: madbd5（デフォルト、RP2040, 5x16, 60キー）、madbd34（RP2040, 4x12スプリット, 41キー）
...
### C版との論理的等価性
- アルゴリズム・状態管理ロジックはupstream C版と比較検証してから実装する
- デフォルト値・定数値がupstreamと一致していることを確認する
- 差異がある場合は意図的であることをコメントで明記する

### デッドコード・未使用変数の禁止
- 到達不可能なコードパスは削除する
- 未使用変数は宣言しないか `_ = var` で明示的に捨てる
- 未使用のインポートを残さない

### 状態リセットの完全性
- 機能の初期化・有効化時には関連する状態変数をすべてリセットする
- 外部から有効化される経路も考慮し、stale state を残さない

### マジックナンバーの禁止
- HID、EEPROM、レジスタアドレス等の定数は名前付き定数にする
- ビットマスクやオフセット値にはコメントまたは定数名で意図を明記する

### 条件式の等価性検証
- 比較対象のユニット（単位）と型が一致していることを確認する
- 常に true/false になる条件式がないか確認する
- C版との動作を照合してから実装する

### 副作用を伴う計算関数の禁止
- 値を計算・変換する関数はグローバル状態を変更しない
- 状態変更は呼び出し側で明示的に行う
