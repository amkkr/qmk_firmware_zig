...
// ============================================================
// USB Event Queue (ISR → main loop)
// ============================================================

/// USB events generated by the ISR and processed by task() in the main loop.
/// C版 usb_main.c の usbevent_t に相当。
pub const UsbEvent = enum(u8) {
    bus_reset,
    setup_req,
    buff_status,
    dev_suspend,
...
/// Lock-free SPSC (Single Producer Single Consumer) ring buffer for USB events.
/// ISR enqueues events; task() dequeues and dispatches them.
/// Size must be a power of 2 for efficient masking.
pub const UsbEventQueue = struct {
    pub const CAPACITY = 16;
    const MASK = CAPACITY - 1;

    buf: [CAPACITY]UsbEvent = .{.bus_reset} ** CAPACITY,
    /// Write index (only modified by ISR / producer)
    head: u8 = 0,
    /// Read index (only modified by task / consumer)
    tail: u8 = 0,

    /// Volatile read of head index (ensures ISR writes are visible to main loop)
    inline fn readHead(self: *UsbEventQueue) u8 {
        if (is_freestanding) {
            return @as(*volatile u8, @ptrCast(&self.head)).*;
        }
        return self.head;
    }

    /// Volatile write of head index
    inline fn writeHead(self: *UsbEventQueue, val: u8) void {
        if (is_freestanding) {
            @as(*volatile u8, @ptrCast(&self.head)).* = val;
        } else {
            self.head = val;
        }
    }

    /// Volatile read of tail index (ensures main loop writes are visible to ISR)
    inline fn readTail(self: *UsbEventQueue) u8 {
        if (is_freestanding) {
            return @as(*volatile u8, @ptrCast(&self.tail)).*;
        }
        return self.tail;
    }

    /// Volatile write of tail index
    inline fn writeTail(self: *UsbEventQueue, val: u8) void {
        if (is_freestanding) {
            @as(*volatile u8, @ptrCast(&self.tail)).* = val;
        } else {
            self.tail = val;
        }
    }

    /// Enqueue an event. Called from ISR context.
    /// Returns false if the queue is full (event dropped).
    pub fn enqueue(self: *UsbEventQueue, event: UsbEvent) bool {
        const h = self.readHead();
        const next_head = (h +% 1) & MASK;
    ...
    /// Dequeue an event. Called from main loop context.
    /// Returns null if the queue is empty.
    pub fn dequeue(self: *UsbEventQueue) ?UsbEvent {
        const t = self.readTail();
        if (t == self.readHead()) {
    ...
    pub fn isEmpty(self: *UsbEventQueue) bool {
        return self.readTail() == self.readHead();
    }

    /// Number of events in the queue.
    pub fn count(self: *UsbEventQueue) u8 {
        return (self.readHead() -% self.readTail()) & MASK;
    }

    /// Reset the queue to empty state.
    pub fn reset(self: *UsbEventQueue) void {
        self.writeHead(0);
        self.writeTail(0);
    }
};

/// Global event queue shared between ISR and main loop.
...
pub fn usbctrlIrqHandler() callconv(.c) void {
    if (is_freestanding) {
        ...
        if (ints & IntBit.DEV_SUSPEND != 0) {
            // Clear DEV_SUSPEND in SIE_STATUS (W1C) — edge-triggered, safe to clear immediately
            @as(*volatile u32, @ptrFromInt(USBCTRL_REGS_BASE + Reg.SIE_STATUS)).* = SieStatus.SUSPENDED;
            _ = event_queue.enqueue(.dev_suspend);
        }
        if (ints & IntBit.DEV_RESUME_FROM_HOST != 0) {
            // Clear DEV_RESUME in SIE_STATUS (W1C) — edge-triggered, safe to clear immediately
            @as(*volatile u32, @ptrFromInt(USBCTRL_REGS_BASE + Reg.SIE_STATUS)).* = SieStatus.RESUME;
            _ = event_queue.enqueue(.dev_resume);
        }
    }
}
...
pub const UsbDriver = struct {
    ...
    cdc_rx_head: u8 = 0,
    cdc_rx_tail: u8 = 0,
    /// Mock CDC RX data (for testing CDC data endpoint OUT reception)
    ...
    pub fn task(self: *UsbDriver) void {
        if (!is_freestanding) {
            // Test mode: convert mock_ints to events for existing test compatibility
            const ints = self.mock_ints;
            if (ints != 0) {
                self.mock_ints = 0;
                if ((ints & IntBit.BUS_RESET) != 0) _ = event_queue.enqueue(.bus_reset);
                if ((ints & IntBit.SETUP_REQ) != 0) _ = event_queue.enqueue(.setup_req);
                if ((ints & IntBit.BUFF_STATUS) != 0) _ = event_queue.enqueue(.buff_status);
                if ((ints & IntBit.DEV_SUSPEND) != 0) _ = event_queue.enqueue(.dev_suspend);
                if ((ints & IntBit.DEV_RESUME_FROM_HOST) != 0) _ = event_queue.enqueue(.dev_resume);
            }
        }

        // Drain all pending events from the queue
        while (event_queue.dequeue()) |ev| {
            switch (ev) {
                .bus_reset => self.handleBusReset(),
                .setup_req => self.handleSetupFromHw(),
                .buff_status => self.handleBuffStatus(),
                .dev_suspend => self.handleDevSuspend(),
                .dev_resume => self.handleDevResume(),
            }
        }
    ...
    pub fn cdcRead(self: *UsbDriver, buf: []u8) u8 {
        ...
        const read_len: u8 = @intCast(@min(buf.len, @as(usize, avail)));
        for (0..read_len) |i| {
    ...
    fn handleCdcRxData(self: *UsbDriver) void {
        ...
        for (0..read_len) |i| {
            ...
            const byte = if (is_freestanding) blk: {
                const dpram_buf = @as([*]volatile u8, @ptrFromInt(USBCTRL_DPRAM_BASE + DPRAM.EP6_OUT_BUF));
    ...
    /// BUFF_STATUS EP6 OUT bit (bit 13): CDC Data OUT transfer complete
    pub const BUFF_STATUS_EP6_OUT: u32 = 1 << 13; // EP6 OUT = bit 2*6+1 = 13
    ...
    fn handleBuffStatus(self: *UsbDriver) void {
        ...
        if ((status & BUFF_STATUS_EP6_OUT) != 0) {
...
test "CDC RX receives data via handleBuffStatus EP6 OUT" {
    var drv = UsbDriver{};
    drv.init();
    drv.state = .configured;

    // Simulate host sending 5 bytes on EP6 OUT
    ...
    drv.mock_buff_status = UsbDriver.BUFF_STATUS_EP6_OUT;
...
test "BUFF_STATUS_EP6_OUT bit position" {
    try testing.expectEqual(@as(u32, 1 << 13), UsbDriver.BUFF_STATUS_EP6_OUT);
}
...
test "UsbEventQueue: enqueue and dequeue single event" {
    var q = UsbEventQueue{};
    try testing.expect(q.isEmpty());
    try testing.expectEqual(@as(u8, 0), q.count());

    try testing.expect(q.enqueue(.bus_reset));
    try testing.expect(!q.isEmpty());
    try testing.expectEqual(@as(u8, 1), q.count());

    const ev = q.dequeue();
    try testing.expect(ev != null);
    try testing.expectEqual(UsbEvent.bus_reset, ev.?);
    try testing.expect(q.isEmpty());
}

test "UsbEventQueue: dequeue from empty returns null" {
    var q = UsbEventQueue{};
    try testing.expectEqual(@as(?UsbEvent, null), q.dequeue());
}

test "UsbEventQueue: enqueue multiple events preserves FIFO order" {
    var q = UsbEventQueue{};
    try testing.expect(q.enqueue(.bus_reset));
    try testing.expect(q.enqueue(.setup_req));
    try testing.expect(q.enqueue(.buff_status));
    try testing.expectEqual(@as(u8, 3), q.count());

    try testing.expectEqual(UsbEvent.bus_reset, q.dequeue().?);
    try testing.expectEqual(UsbEvent.setup_req, q.dequeue().?);
    try testing.expectEqual(UsbEvent.buff_status, q.dequeue().?);
    try testing.expectEqual(@as(?UsbEvent, null), q.dequeue());
}

test "UsbEventQueue: full queue rejects new events" {
    var q = UsbEventQueue{};
    // Fill to capacity - 1 (ring buffer uses one slot as sentinel)
    for (0..UsbEventQueue.CAPACITY - 1) |_| {
        try testing.expect(q.enqueue(.setup_req));
    }
    // Queue is now full; next enqueue should fail
    try testing.expect(!q.enqueue(.bus_reset));
    try testing.expectEqual(@as(u8, UsbEventQueue.CAPACITY - 1), q.count());
}

test "UsbEventQueue: reset clears queue" {
    var q = UsbEventQueue{};
    try testing.expect(q.enqueue(.bus_reset));
    try testing.expect(q.enqueue(.setup_req));
    try testing.expect(!q.isEmpty());
...
test "UsbEventQueue: wrap-around behavior" {
    var q = UsbEventQueue{};
...
test "task dispatches events from queue via mock_ints" {
    event_queue.reset();
    var drv = UsbDriver{};
    drv.init();
    drv.state = .addressed;

    // Simulate ISR setting BUS_RESET via mock_ints
    drv.mock_ints = IntBit.BUS_RESET;
    drv.task();

    // BUS_RESET handler should have reset the driver state
    try testing.expectEqual(DeviceState.default_state, drv.state);
    try testing.expectEqual(@as(u8, 0), drv.address);
    try testing.expect(event_queue.isEmpty());
}

test "task processes multiple events in single call" {
    event_queue.reset();
    var drv = UsbDriver{};
    drv.init();
    drv.state = .configured;
    ...
    // Directly enqueue multiple events (simulating ISR behavior)
    _ = event_queue.enqueue(.bus_reset);
    _ = event_queue.enqueue(.bus_reset);

    drv.task();

    // Both bus resets should have been processed
    try testing.expect(event_queue.isEmpty());
    try testing.expectEqual(DeviceState.default_state, drv.state);
}
...
test "NVIC constants are correct for USBCTRL_IRQ" {
    ...
    // IRQ5 bit mask = 1 << 5 = 32
    try testing.expectEqual(@as(u32, 32), @as(u32, 1) << 5);
    // IPR register for IRQ5: IRQ4-7 are in IPR[1] at offset 4
    try testing.expectEqual(@as(u32, 0xE000E404), NVIC_IPR_BASE + 4);
}

test "usbctrlIrqHandler has correct calling convention" {
    // Verify the ISR function pointer is compatible with the vector table
    const handler_ptr: *const fn () callconv(.c) void = &usbctrlIrqHandler;
    try testing.expect(@intFromPtr(handler_ptr) != 0);
}
