...
/// キーマップ参照関数を注入する。
/// production startup と test setup から呼ぶ。
pub fn setKeymapLookup(lookup: KeymapLookupFn) void {
...
/// production code には影響しない (test_fixture.zig の `fixture_test_keymap` とは
/// 独立した別の static 変数で、 名前空間としても明確に分離している)。
/// 命名規則: `kb_*` prefix は keyboard.zig 内テスト専用 storage を表す (Issue #402)。
var kb_test_keymap: keymap_mod.Keymap = keymap_mod.emptyKeymap();

fn kbTestKeymapLookup(l: u5, row: u8, col: u8) Keycode {
    return keymap_mod.keymapKeyToKeycode(&kb_test_keymap, l, row, col);
}

fn setup() *TestMockDriver {
    mock_driver = .{};
    kb_test_keymap = keymap_mod.emptyKeymap();
    ...
    setKeymapLookup(kbTestKeymapLookup);
    return &mock_driver;
}
...
test "keyboard_task: 単一キー押下→リリースでHIDレポートが正しく生成される" {
    ...
    // (0,0) に KC_A を配置
    kb_test_keymap[0][0][0] = keycode.KC.A;
...
test "keyboard_task: 修飾キーがmodsに正しく反映される" {
    ...
    // (0,0) に LSHIFT を配置
    kb_test_keymap[0][0][0] = keycode.KC.LEFT_SHIFT;
...
test "keyboard_task: MO()レイヤー切替が動作する" {
    ...
    // Layer 1: (0,1) = KC_B
    kb_test_keymap[0][0][0] = keycode.MO(1);
    kb_test_keymap[0][0][1] = keycode.KC.A;
    kb_test_keymap[1][0][1] = keycode.KC.B;
...
test "keyboard_task: TD()タップダンスがパイプライン経由で動作する" {
    ...
    // (0,0) に TD(0) を配置
    kb_test_keymap[0][0][0] = keycode.TD(0);
...
test "keyboard_task: Caps Word トグルが動作する" {
    ...
    kb_test_keymap[0][0][0] = keycode.CW_TOGG;
    kb_test_keymap[0][0][1] = keycode.KC.A;
...
test "keyboard_task: Repeat Key が直前のキーを再送する" {
    ...
    // (0,0) = KC_A, (0,1) = QK_REP
    kb_test_keymap[0][0][0] = keycode.KC.A;
    kb_test_keymap[0][0][1] = keycode.QK_REP;
...
test "keyboard_task: Layer Lock がレイヤーをロックする" {
    ...
    kb_test_keymap[0][0][0] = keycode.MO(1);
    kb_test_keymap[0][0][1] = keycode.KC.A;
    kb_test_keymap[1][0][1] = keycode.QK_LLCK;
...
test "keyboard_task: Layer Lock アイドルタイムアウトが他キー入力でリセットされる" {
    ...
    kb_test_keymap[0][0][1] = keycode.KC.A;
    kb_test_keymap[1][0][1] = keycode.QK_LLCK;
    kb_test_keymap[1][0][2] = keycode.KC.B;
...
test "keyboard_task: Tri Layer — Lower+Upper で Adjust が有効になる" {
    ...
    kb_test_keymap[0][0][0] = keycode.TL_LOWR;
    kb_test_keymap[0][0][1] = keycode.TL_UPPR;
    // 上位レイヤーはフォールスルーさせる
    for (1..keymap_mod.MAX_LAYERS) |l| {
        kb_test_keymap[l][0][0] = keycode.KC.TRNS;
        kb_test_keymap[l][0][1] = keycode.KC.TRNS;
    }
