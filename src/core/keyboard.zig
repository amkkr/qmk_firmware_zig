...
fn resolveKeycode(ev: KeyEvent) Keycode {
    ...
    return keymap_mod.keymapKeyToKeycode(keymap_state.getKeymap(), use_layer, ev.key.row, ev.key.col);
}
...
test "keyboard_task: 単一キー押下→リリースでHIDレポートが正しく生成される" {
    ...
    // (0,0) に KC_A を配置
    keymap_state.getKeymap()[0][0][0] = keycode.KC.A;
...
test "keyboard_task: 修飾キーがmodsに正しく反映される" {
    ...
    // (0,0) に LSHIFT を配置
    keymap_state.getKeymap()[0][0][0] = keycode.KC.LEFT_SHIFT;
...
test "keyboard_task: MO()レイヤー切替が動作する" {
    ...
    // Layer 1: (0,1) = KC_B
    keymap_state.getKeymap()[0][0][0] = keycode.MO(1);
    keymap_state.getKeymap()[0][0][1] = keycode.KC.A;
    keymap_state.getKeymap()[1][0][1] = keycode.KC.B;
...
test "keyboard_task: TD()タップダンスがパイプライン経由で動作する" {
    ...
    // (0,0) に TD(0) を配置
    keymap_state.getKeymap()[0][0][0] = keycode.TD(0);
...
test "keyboard_task: Caps Word トグルが動作する" {
    ...
    keymap_state.getKeymap()[0][0][0] = keycode.CW_TOGG;
    keymap_state.getKeymap()[0][0][1] = keycode.KC.A;
...
test "keyboard_task: Repeat Key が直前のキーを再送する" {
    ...
    // (0,0) = KC_A, (0,1) = QK_REP
    keymap_state.getKeymap()[0][0][0] = keycode.KC.A;
    keymap_state.getKeymap()[0][0][1] = keycode.QK_REP;
...
test "keyboard_task: Layer Lock がレイヤーをロックする" {
    ...
    keymap_state.getKeymap()[0][0][0] = keycode.MO(1);
    keymap_state.getKeymap()[0][0][1] = keycode.KC.A;
    keymap_state.getKeymap()[1][0][1] = keycode.QK_LLCK;
...
test "keyboard_task: Layer Lock アイドルタイムアウトが他キー入力でリセットされる" {
    ...
    keymap_state.getKeymap()[0][0][1] = keycode.KC.A;
    keymap_state.getKeymap()[1][0][1] = keycode.QK_LLCK;
    keymap_state.getKeymap()[1][0][2] = keycode.KC.B;
...
test "keyboard_task: Tri Layer — Lower+Upper で Adjust が有効になる" {
    ...
    keymap_state.getKeymap()[0][0][0] = keycode.TL_LOWR;
    keymap_state.getKeymap()[0][0][1] = keycode.TL_UPPR;
    // 上位レイヤーはフォールスルーさせる
    for (1..keymap_mod.MAX_LAYERS) |l| {
        keymap_state.getKeymap()[l][0][0] = keycode.KC.TRNS;
        keymap_state.getKeymap()[l][0][1] = keycode.KC.TRNS;
    }
