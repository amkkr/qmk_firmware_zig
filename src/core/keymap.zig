...
// ============================================================
// resolveKeycode: Layer 4-15 boundary 検証
// ============================================================
//
// MAX_LAYERS = 16 のため Layer 0-15 まで利用可能。
// 既存テストは Layer 0-3 のみ検証していたが、 madbd5 は Layer 7 まで使用し、
// Layer 15 は MAX_LAYERS-1 の境界値。
// LayerState は u32 なので 1 << 15 を直接シフトできる。

test "resolveKeycode: Layer 7 (madbd5 最大レイヤー) アクティブ" {
    var km = emptyKeymap();
    km[7][0][0] = KC.A;

    // Layer 7 のみアクティブ
    try testing.expectEqual(KC.A, resolveKeycode(&km, 1 << 7, 0, 0));
}

test "resolveKeycode: Layer 15 (MAX_LAYERS-1 boundary) アクティブ" {
    var km = emptyKeymap();
    km[15][0][0] = KC.A;

    // Layer 15 のみアクティブ
    try testing.expectEqual(KC.A, resolveKeycode(&km, 1 << 15, 0, 0));
}

test "resolveKeycode: Layer 7 + Layer 15 透過 → Layer 7 へフォールスルー" {
    var km = emptyKeymap();
    km[7][0][0] = KC.A;
    km[15][0][0] = KC.TRNS;

    // Layer 7+15 アクティブ → Layer 15 透過 → Layer 7 の A
    try testing.expectEqual(KC.A, resolveKeycode(&km, (1 << 7) | (1 << 15), 0, 0));
}
