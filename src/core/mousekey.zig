...
fn wheelUnitKinetic() u8 {
    ...
    if (mousekey_accel & (1 << 0) != 0) {
        ...
        // 既に最大速度に達している場合は speed を base_movements に維持する。
        if (kinetic_wheel_interval > 1000 / kinetic_speed_config.wheel_base_movements) {
            const time_elapsed: u32 = @as(u32, timer.elapsed(mouse_timer)) / 50;
...
test "kinetic_speed - ホイール最大速度到達後に加速が停止する" {
    // 最大速度 (wheel_base_movements) に到達した後、
    // wheelUnitKinetic を呼んでも kinetic_wheel_interval が変化しないことを確認する。
    _ = setupTest();
    defer teardownTest();
    ...
    // 十分な時間を経過させて最大速度に到達させる。
    // speed = initial(16) + time_elapsed + time_elapsed^2/2 が base(32) を超えるまで進める。
    // time_elapsed = timer.elapsed(mouse_timer) / 50 なので、大きな時間を進める。
    // 1000ms 進めれば time_elapsed = 1000/50 = 20 → speed = 16 + 20 + 200 = 236 >> 32
    timer.mockAdvance(1000);
    task();

    const interval_at_max = getKineticWheelInterval();
    // 最大速度: 1000 / wheel_base_movements = 1000 / 32 = 31
    try testing.expectEqual(@as(u16, 1000 / 32), interval_at_max);

    // さらに時間を進めて再度 task() を呼んでも interval が変化しないことを確認。
    timer.mockAdvance(500);
    task();

    const interval_still_at_max = getKineticWheelInterval();
    try testing.expectEqual(interval_at_max, interval_still_at_max);
