...
test "getHighestLayer: Layer 14, 15 同時 ON 時の優先度" {
    // 引数のみ参照する純粋関数なので resetState 不要
    // 0xC000 = (1<<14) | (1<<15) — 上位 2 レイヤー同時 ON 時に 15 を返す
    const both: LayerState = (@as(LayerState, 1) << 14) | (@as(LayerState, 1) << 15);
    try testing.expectEqual(@as(LayerState, 0xC000), both);
    try testing.expectEqual(@as(u5, 15), getHighestLayer(both));

    // 0x4000 = 1<<14 — Layer 14 単独
    const only14: LayerState = @as(LayerState, 1) << 14;
    try testing.expectEqual(@as(LayerState, 0x4000), only14);
    try testing.expectEqual(@as(u5, 14), getHighestLayer(only14));
}

test "getHighestLayer: 全 Layer 1-15 同時 ON 時の優先度" {
    // 0xFFFE = Layer 1-15 全 ON, Layer 0 のみ OFF
    // 上位 bit から走査するため Layer 15 が最初に発見される
    var all_high: LayerState = 0;
    var bit: u5 = 1;
    while (bit < MAX_LAYERS) : (bit += 1) {
        all_high |= @as(LayerState, 1) << bit;
    }
    try testing.expectEqual(@as(LayerState, 0xFFFE), all_high);
...
test "layerSwitchGetLayer: Layer 14, 15 が両方透過時 Layer 0 fallback" {
    resetState();
    defer resetState();
    // resetState() が default_layer_state = 1 (Layer 0) を設定済み

    // Layer 14, 15 とも透過 -> Layer 0 にフォールバックする検証
    const keymapFn = struct {
        fn f(l: u5, _: u8, _: u8) Keycode {
            return switch (l) {
                0 => keycode.KC.A,
...
test "layerSwitchGetLayer: Layer 15 非透過、Layer 14 透過時は Layer 15 選択" {
    resetState();
    defer resetState();
    // resetState() が default_layer_state = 1 (Layer 0) を設定済み

    // Layer 15 が非透過なら Layer 14 まで降りずに即 15 が選ばれる
    const keymapFn = struct {
        fn f(l: u5, _: u8, _: u8) Keycode {
            return switch (l) {
                0 => keycode.KC.A,
    ...
    layerOn(14);
    layerOn(15);
    try testing.expectEqual(@as(u5, 15), layerSwitchGetLayer(keymapFn, 0, 0));
}

test "layerSwitchGetLayer: default_layer_state を Layer 2 にした時 Layer 2 が選ばれる" {
    resetState();
    defer resetState();

    // default を Layer 2 に変更 (defaultLayerSet 経由でコールバックも通す)
    defaultLayerSet(@as(LayerState, 1) << 2);

    // Layer 14, 15 透過 -> default Layer 2 (非透過) で停止する
    const keymapFn = struct {
        fn f(l: u5, _: u8, _: u8) Keycode {
            return switch (l) {
                0 => keycode.KC.A,
...
test "layerSwitchGetLayer: KC.NO は非透過として Layer が選ばれる" {
    resetState();
    defer resetState();

    // KC.NO は ACTION_NO (action_code.zig:89) であり ACTION_TRANSPARENT (= 0x0001) ではない。
    // よって layerSwitchGetLayer は Layer 14 で停止し、Layer 0 まで降りない。
    const keymapFn = struct {
        fn f(l: u5, _: u8, _: u8) Keycode {
            return switch (l) {
                0 => keycode.KC.A,
    ...
    layerOn(14);
    layerOn(15);
    try testing.expectEqual(@as(u5, 14), layerSwitchGetLayer(keymapFn, 0, 0));
}
